...
- [ ] **병종 다양화**: 진짜 7번째 유닛 추가는 `engine.js UNITS`·`verify.js NAMES`·6유닛 삼각 밸런스 전체에 영향 — **여전히 미정**, 필요성이 명확해지면 재검토(현재로선 파성추만으로 공성 밸런스 충분해 보임).
...
## 지금 다음 액션
**Milestone A·B·C 전부 완료**(2026-07-18). B3(안개·탐험)만 사용자 요청으로 계속 보류. 남은 후보:
- **C3 병종 다양화**(진짜 7번째 유닛) — 필요성이 뚜렷해지면(공성 소모품만으로 후반 수성이 여전히 과함 등) 재검토.
