...
## Milestone G — 캐주얼 루프 튜닝 (자동사냥·자동생산·보상·전쟁 참여)

**플레이테스트 진단 기반**(2026-07-18): 실제로 새 게임을 71턴 플레이해보니 비전("느긋한 PvE 성장 + 이벤트성 전쟁")과 체감이 어긋남 — 성장 정체(국력 98→108), 부대가 킬 후 방치, 보상 트리클(경험치1·용린0), 전쟁이 관전. 사용자가 A·C·D·F 4개 지정, A는 "이길 만한 건 다"(자동 강약 판정)·F는 수동 "전군 소집" 확정. 전부 engine.js 무변경.

### G-D. 보상 튜닝 ✅ (2026-07-18)
- [x] 몬스터 보상 테이블 상향 — 사냥이 0을 주던 용린·설계도를 1+로: `XP_REWARD 사냥1→2/토벌2→4/레이드3→8`, `DRAGON_SCALE 사냥0→1/토벌2→4/레이드8→16`, `LORD_XP ~2배`, `MATERIAL ~2배`, `BLUEPRINT 사냥0→1`. 자동사냥(G-A)이 빈도를 올리므로 per-kill은 "초라하지 않은" 선까지만.
- [x] **적(E) 위협군 처치 보상 신설** — `threatKillReward(g,foe,sum)`: P가 습격대·시즌침공·원정대를 격파(야전 `res.w=A & defender.side=E`, 수성 `res.w=B & attacker.side=E`)하면 병력 규모 비례 자원(식량n·철0.6n·목재0.4n) + 경험치·군주경험치. 정복은 checkVictory가 별도 처리. **방어 승리가 남는 장사가 됨.**
- **검증**: `node verify.js` 12 PASS. node 스모크로 사냥 킬 보상 상향(xp2·용린1·재료2·설계도1) 확인, 시즌 침공(30명) 격퇴 시 식량30·철18·목재12·경험치3·군주경험치3 지급 확인. `node sim.js` — 봇도 방어 보상을 받아 국력 442→471·함락 4.1→3.2로 소폭 강화(의도된 효과, 회귀 아님). 목재 3049 잉여는 G-C(자동생산 소비처) 필요성 재확인.

### G-A. 부대 자동 사냥 ✅ (2026-07-18)
- [x] `a.hunt` 토글 + `setHunt(g,armyId,on)`. **강약 판정**: `unitScore=(hp+atk*4+df*2)*tierMult`, `armyPower=compPower×(1+전투버프추정)`(영웅등급+traitSum+lord+dragon 근사), `nearestWinnableMonster`가 `armyPower ≥ 몬스터power×1.2`(안전 마진)인 최근접 M군 반환(레이드 보스 제외).
- [x] `assignHuntOrders(g)`를 `endTurn`의 `moveTick` 직전 호출 — hunt 켠 유휴 부대가 승산 몬스터로 자동 진군, 킬 후 유휴→다음 목표 연쇄. `offlineStep`엔 미연결(오프라인 전투 금지 원칙).
- [x] `ui.js` — 부대 패널 "⚔ 자동 사냥 ON/OFF" 토글.
- **검증**: `node verify.js` 12 PASS. **replay 100틱**: 사냥대(전력 2718)가 이길 수 있는 약한 둥지(늑대1584·도적1392)만 20회 연쇄 사냥, 토벌(3084·4284)·레이드(6408)는 강약 판정으로 스킵, **부대 전멸 0**. 화폐 폭증 — 경험치41·용린20·재료40·설계도20·군주Lv3(baseline은 경험치1·용린0·Lv1) → **깊은 시스템이 실제로 시동 걸림**. `node sim.js` 469(봇 미사용, 불변). 브라우저 토글 동작 확인.
- **다음**: G-C(자동 생산).

### G-C. 자동 생산 ✅ (2026-07-18)
- [x] `castle.autoProduce`(건물→{u,tier}) + `setAutoProduce` 토글. `autoProduceTick(g)`가 매 틱 반복생산 건물의 큐를 `buildRate`만큼 채움(자원 되는 한, `produce` 재사용) → `endTurn`·`offlineStep` 양쪽에서 `tickProduction` 직전 호출. 자원 부족 시 조용히 멈춤(성장이 자원에 자연 게이트).
- [x] `ui.js` — 건물 탭 병종별 "🔁" 반복생산 토글 + 활성 안내. `applySave` 호환.
- [x] 효과: **자원→병력→국력** 전환으로 저입력 성장. 굶주림 시 생산 자동 정지 → 식량 지속가능 병력 규모에서 자기 균형.
- **검증**: `node verify.js` 12 PASS. **종합 replay**(자동사냥+자동생산+농장, 70턴): 국력 **108→165**(baseline +10 대비 +57), **식량 426폭증→0**(잉여가 병력으로 소진), 총병력 16→105. 자동생산만 격리 테스트도 국력 98→119·주둔군 52. `node sim.js` 477(봇은 autoProduce 미사용, 불변).
- **다음**: G-F(전쟁 참여).

### G-F. 전쟁 참여 ✅ (2026-07-18)
- [x] `rallyToDefense(g)` — 집(P)에 없는 모든 P부대를 `orderMove→"P"` + `a.hunt=false`(귀환 후 다시 사냥 안 나가게). 귀환 부대는 P에서 `defendCastle`이 자동 수비 합류(F2). 반환: 소집 부대 수.
- [x] `ui.js` — 시즌 예고(warned) + 나간 부대 있을 때 시즌 패널에 **"⚔ 전군 소집 (N개 부대 귀환)"** 버튼. 예고 12턴 창구에 능동 대응.
- [x] 방어 승리 보상은 G-D의 `threatKillReward`가 담당 — 시즌 침공 격퇴 시 전투 모달 결과에 "격퇴 보상 ..." 자동 표시.
- **검증**: `node verify.js` 12 PASS. node 스모크로 rally가 사냥대 dest=P·hunt=false로 전환 확인, 강한 주둔군이 습격대 격퇴하며 방어 보상(경험치·군주xp) 누적 확인. 브라우저에서 예고 시 소집 버튼 노출 → 클릭 → 부대 dest가 PR0→P로 전환·자동사냥 해제 확인.
- **Milestone G(캐주얼 루프 튜닝, D·A·C·F) 전체 완료.**

## Milestone H — 방치형 전환 (2026-07-18)
사용자 방향 확정: "수동조작이 가미된 자동 방치형" — 느긋하게 접속, 대부분 알아서 굴러가고 전쟁만 손으로. 스태미나(행동력)는 **안 넣음**(마찰 제거가 방향).

### H1. 자동 채집 배치 ✅
- [x] `a.gather` 토글(사냥과 상호배타). `assignGatherOrders`/`bestGatherNode` — 유휴 부대를 가장 부족한 자원의 미점유 자원지로 dijkstra 최근접 파견, 도착 시 기존 `gatherOf` 수입. `endTurn`에서 moveTick 전 호출. 전군 소집 시 채집도 해제.
- [x] `ui.js` — 부대 패널 "⛏ 자동 채집" 토글.
- **검증**: verify 12 PASS. node: 최저 자원지(STONE) 직행·정착 확인.

### H2. 오프라인 사냥 + 확률적 견제 전투 ✅
- [x] 오프라인이 경제만 → **PvE 사냥 + 견제 전투**까지 정산. 대규모 전쟁(시즌·AI원정대)·수도 공방은 오프라인 제외(자리 비운 사이 함락 방지 — 전쟁은 접속해서).
- [x] 사냥: `a.hunt` 부대가 이길 만한 몬스터를 쿨다운(`OFF_HUNT_COOLDOWN=600`틱, ~시간당 2~3회)으로 처치. 보상·부상은 온라인과 동일 경로(`grantMonsterReward` — resolveBattle에서 추출한 공용 헬퍼).
- [x] 견제(유저 요청): 밖에 나간 채집·사냥 부대가 낮은 확률(`OFF_HARASS_CHANCE=0.00012`×노출부대수)로 습격받아 간헐 전투. 견제군=기회주의적 약탈대(목표 전력 20~50%, `pickAIUnit(null)`) → **병력 구성으로 승패**(전열형 방어 96%+, 궁병만 76%). 수도 수비대는 오프라인 안전.
- [x] **승산 판정 버그 수정**: `nearestWinnableMonster`가 전력근사(hp+atk*4+df*2)로 공/방 감쇄를 못 담아 저공격·고방어 부대를 "압승" 오판→전멸. 실제 전투 엔진 3판 시뮬(`huntWinnable`)로 확정 — 온라인 자동사냥 침묵 전멸도 동시 방지.
- **검증**: verify 12 PASS. node 튜닝: 8h에 사냥~17·견제~4·구성별 3부대 생존차(균형 21/30·전열형 29/30). sim 375(봇 미사용 불변).

### H4. 접속 요약 리포트 ✅
- [x] `offlineTick`이 사냥 수·보상·견제 승패·부대 손실·건설/연구 완료·국력 증가·자원 순증을 리포트로 반환 → "🌙 자리를 비운 사이" 모달에 표시. 오프라인 상한 12h→**24h**(스태미나 없음, 상한만).
- **검증**: 브라우저에서 3h 오프라인 모달 확인(자원·자동사냥 4회+보상·국력).
- (H3 연구 자동대기열은 이번 범위 제외 — 사용자 선택.)

## Milestone I — 끝없는 경쟁 레이어 (2026-07-18)
사용자 방향 확정: **완주=끝내는 게 아니라 아발론처럼 끝없이 성장·경쟁·전투**. 엔딩 없음(비종결 유지). 방치 성장(H)이 힘을 기르는 엔진, 그 위에 경쟁 레이어. (영토 4X 확장은 미채택)

### I1. 라이벌 왕국 + 대륙 랭킹 ✅
- [x] `RIVALS` const(강철왕 발트 등 4왕국, factor·병종개성) ↔ `g.rivals`. `rivalTick`: 각 라이벌이 플레이어 국력의 배수(factor)를 목표로 서서히 수렴 → 항상 경쟁권(오프라인 폭주 없음), factor>1은 방치 시 추월. `continentalRankings`/`myRank`. `endTurn`·`offlineStep`에서 tick.
- [x] ui: 상단 "🏆 대륙 #N/M" 배지 + 아코디언 랭킹 막대표. applySave 호환.
- **검증**: verify 12 PASS. node: 능동 성장 3→2위, 방치 시 밀림 확인.

### I2. 웨이브 라이벌 태깅 + 격파→순위 ✅
- [x] `pickAggressorRival`(강한 라이벌일수록 자주 공격), AI 원정대·시즌 대침공에 `e.rival` 태그+이름 접두("화염군주 카산의 원정대"). `damageRival`: 라이벌 웨이브 야전 격파/수성 격퇴 시 규모 비례 국력 감소+`rivalKills`. 방치 시 `rivalTick`이 회복 → 꺾으면 순위↑/방치하면 추월.
- **검증**: verify 12 PASS. node: 태그 웨이브 격파→라이벌 국력 억제·순위 상승.

### I3. 경쟁 시즌 — 점수·티어·보상 ✅
- [x] 대침공 스케줄러(`g.season`)와 분리된 `g.rankSeason`. `RANK_TIERS`(브론즈~전설)·`RANK_SEASON_LEN=180`·`addSeasonScore`·`rankSeasonTick`(순차 tick). 점수 훅: 위협 격파/방어(규모 비례)·라이벌 격파(+25)·몬스터 처치(티어별 소액)·마일스톤(+40). 마감: 티어 보상+`0.35 소프트리셋`+`num++`, **over 절대 안 켬**. ui: 경쟁 시즌 패널 + 마감 배너(재생 안 끊음).
- **검증**: verify 12 PASS. node: 마감→티어·보상·소프트리셋·num++·over=false.

### I4. 위협 등반 escalation ✅ (sim 게이트)
- [x] `threatMul`=`min(1.5, 1+0.06*(시즌회차-1)+0.03*랭크티어)`. `seasonTick`/`factionTick` need + `rivalTick` 목표에 곱 → 진행에 따라 세계가 서서히 강해짐(상한).
- **검증**: `node sim.js` 게이트 통과 — 국력 388·함락 6.31(기존 범위), 발생 횟수 불변(규모만 상승). verify 12 PASS.

### I5. 폴리시·오프라인 연동·문서 ✅
- [x] `offlineTick` 리포트에 시즌 마감 횟수·대륙 순위 변동(#N→#M, 추월당함 경고). 구버전 세이브 회귀(rivals/rankSeason 자동 복원) 확인. ROADMAP 기록.

## 지금 다음 액션
**Milestone A·B·C·E·F·G·H·I 완료**(2026-07-18). I로 끝없는 경쟁 레이어: 라이벌 4왕국+대륙 랭킹(I1)·웨이브 태깅+격파→순위(I2)·경쟁 시즌 점수/티어/보상(I3)·위협 등반(I4). 방치로 힘을 길러 라이벌·랭킹을 다투고 시즌마다 랭크·보상으로 매듭지으며 무한 등반. **튜닝 노브**(재플레이 조정): 라이벌 `factor`·`RIVAL_HIT`·`RANK_TIERS` 문턱·`RANK_SEASON_LEN`·`RANK_SOFTRESET`·`THREAT_CAP`. B3(안개)·병종 다양화·스태미나·영토4X·엔딩은 미채택. 다음은 재플레이 체감 후 밸런스 재진단 권장.
