...
## 지금 다음 액션
**Milestone A·B·C·E·F·G·H 완료**(2026-07-18). H로 방치형 전환: 자동 채집(H1)·오프라인 사냥+견제전투(H2)·접속 요약(H4). 오프라인이 이제 성장+PvE+간헐 전투까지 굴러가고 병력 구성이 손실을 가른다. **튜닝 노브**(재플레이로 조정): `OFF_HARASS_CHANCE`(견제 빈도)·견제군 전력배율 20~50%·`OFF_HUNT_COOLDOWN`(사냥 속도). B3(안개)·병종 다양화·스태미나는 확정 미채택. 다음은 재플레이 체감 후 재진단 권장.
