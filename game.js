...
(function(global){
  ...
  const RESPAWN_DELAY_RES=10;                       // 몬스터 둥지(8턴)보다 살짝 느긋한 경제형 재생성
  ...
  const buildDur=(kind,lv)=>{ lv=lv||0;
    if(kind==="castle")return 4+Math.floor(lv/3);
    if(kind==="bld")return 3+Math.floor(lv/2);
    if(kind==="wall")return 3+Math.floor(lv/3);
    if(kind==="construct")return 3; if(kind==="univ")return 3; if(kind==="tavern")return 2;
    return 2+Math.floor(lv/2); };
  ...
  function upgradeBuilding(g,key){if(!g.castle.buildings.includes(key))return"미건설";
    const lv=g.castle.blevel[key]||1; if(lv>=tierCap(g))return"최대 레벨(마일스톤 해금 필요)"; return startBuild(g,"bld",key,bUpCost(key,lv),buildDur("bld",lv),`${key} → T${lv+1}`);}
  function fortifyWall(g){return startBuild(g,"wall",null,wallCost(g.castle.wall||0),buildDur("wall",g.castle.wall||0),"성벽 보강");}
  ...
  function levelUp(g){return startBuild(g,"castle",null,castleUpCost(g.castle.level),buildDur("castle",g.castle.level),"성 레벨업");}
