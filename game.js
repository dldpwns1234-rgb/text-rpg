...
(function(global){
  ...
  const buildRate=g=>{const h=cityHero(g);return 1+(g.castle.level-1)+(h?(h.grade>=3?2:1):0)+(hasR(g,"대장간")?1:0)+(h?traitSum(h,"buildBonus"):0);};
  ...
  function econIncome(g){const inc={식량:0,목재:0,석재:0,철:0}, ch=cityHero(g);
    const mult=(hasR(g,"영농")?1.5:1)*(1+(ch?traitSum(ch,"econBonus"):0));   // 감독관 특성: 자원 건물 산출 추가
  ...
  function resolveBattle(g,attacker,defender,node){
    ...
    if(attacker.dragon){ aB+=dragonStageBuff+dragonSkillSum(g,"atkBonus"); if(defender.side==="M") aB+=dragonSkillSum(g,"vsMonsterBonus"); }
    if(defender.dragon) dB+=dragonStageBuff+dragonSkillSum(g,"defBonus");
    let siegeUsed=false;
