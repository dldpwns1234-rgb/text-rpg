...
(function(global){
  ...
  // 아발론 참조: 병력생산/건설/연구 소요시간 + 위협 시계(AI 웨이브/시즌/유예/재등장 주기)를 묵직하게(사용자 확정 2026-07-21, ~×30).
  // 채집/수입은 실측 결과 그대로 둔다(§ 아래) — 건설·훈련이 항상 슬롯 1개 직렬 처리라 자원은 애초에 병목이 아니었고,
  // 수입까지 ×30 하면 소비가 못 따라가 잉여가 턴 12000 기준 수십만 단위로 폭주(실제 sim.js 장기 실행으로 확인) → 비용처럼 그대로 유지.
  const TIME_SCALE=30;
  ...
  // King of Avalon 참조(§21): 슬롯당 등급 곡선을 병종 티어 캡(T1~T5)과 통일 — 슬롯당 5개, 전부 성능이 다른 유일 아이템(재스킨 중복 없음).
  // 기존 T1/T2 아이템 이름·수치는 세이브 하위호환을 위해 그대로 유지, T3~T5만 신규 추가.
  ...
  const EQUIPMENT={
    ...
    "룬 대검":      {slot:"무기",  tier:3,need:{blueprint:4, material:16},cost:{철:80,석재:40,목재:20},field:"atkBonus",amount:0.24,desc:"공격 +24%p"},
    "왕의 검":      {slot:"무기",  tier:4,need:{blueprint:7, material:28},cost:{철:140,석재:70,목재:40},field:"atkBonus",amount:0.35,desc:"공격 +35%p"},
    "용왕의 검":    {slot:"무기",  tier:5,need:{blueprint:12,material:48},cost:{철:240,석재:120,목재:70},field:"atkBonus",amount:0.50,desc:"공격 +50%p"},
    ...
    "용린 갑주":    {slot:"방어구",tier:3,need:{blueprint:4, material:16},cost:{목재:80,철:40,석재:20},field:"defBonus",amount:0.24,desc:"방어 +24%p"},
    "왕실 갑주":    {slot:"방어구",tier:4,need:{blueprint:7, material:28},cost:{목재:140,철:70,석재:40},field:"defBonus",amount:0.35,desc:"방어 +35%p"},
    "용왕의 비늘":  {slot:"방어구",tier:5,need:{blueprint:12,material:48},cost:{목재:240,철:120,석재:70},field:"defBonus",amount:0.50,desc:"방어 +50%p"},
    ...
    "건축가의 투구":{slot:"투구",  tier:3,need:{blueprint:4, material:16},cost:{목재:55,철:45},        field:"buildBonus",amount:3,desc:"건설 속도 +3"},
    "군주의 투구":  {slot:"투구",  tier:4,need:{blueprint:7, material:28},cost:{목재:95,철:80},        field:"buildBonus",amount:4,desc:"건설 속도 +4"},
    "천공의 투구":  {slot:"투구",  tier:5,need:{blueprint:12,material:48},cost:{목재:160,철:135},      field:"buildBonus",amount:6,desc:"건설 속도 +6"},
    ...
    "황금 인장":    {slot:"장신구",tier:3,need:{blueprint:4, material:16},cost:{식량:70,철:45},        field:"econBonus",amount:0.32,desc:"자원 건물 산출 +32%"},
    "왕조의 인장":  {slot:"장신구",tier:4,need:{blueprint:7, material:28},cost:{식량:120,철:80},       field:"econBonus",amount:0.46,desc:"자원 건물 산출 +46%"},
    "영겁의 인장":  {slot:"장신구",tier:5,need:{blueprint:12,material:48},cost:{식량:200,철:135},      field:"econBonus",amount:0.65,desc:"자원 건물 산출 +65%"},
  };
  // King of Avalon 참조: 영웅 "서약 특전"·장비 "스미스의 인" 세트효과와 같은 패턴 — 4슬롯 전부 착용 시(등급 무관) 소량의 전군 세트 보너스.
  const LORD_SET_BONUS={atkBonus:0.05, defBonus:0.05, econBonus:0.05, buildBonus:1};
  const lordEquipSum=(g,field)=>{ let s=0; const eq=(g.lord&&g.lord.equipment)||{}; let filled=0;
    for(const slot of EQUIP_SLOTS){ const itemId=eq[slot]; if(!itemId) continue;
      const it=(g.lordInventory||[]).find(x=>x.id===itemId); if(!it) continue;
      filled++;
      const cat=EQUIPMENT[it.key]; if(cat&&cat.field===field) s+=cat.amount*(1+ENHANCE_MULT[it.enhance||0]); }
    if(filled>=EQUIP_SLOTS.length && LORD_SET_BONUS[field]) s+=LORD_SET_BONUS[field];
    return s; };
  ...
  // TIME_SCALE(아발론 참조 ×30) 적용 — 베이스 테이블은 그대로 두고 노출값만 스케일(드리프트 방지).
  const TRAIN_TICKS_BASE={1:2,2:4,3:7,4:11,5:16};
  const TRAIN_TICKS=Object.fromEntries(Object.entries(TRAIN_TICKS_BASE).map(([t,v])=>[t,v*TIME_SCALE]));
  ...
  // 고티어 = 비용↑, 특히 철(기획서: 고티어 철 병목). 아발론 참조: engine.tierMult을 복리(×1.28)로 가파르게 바꾸면서
  // 여기도 같이 복리(×1.55)로 올려 "고티어=스탯은 세지만 자원 대비 비효율" 비율(T5 기준 구값 1.60/3.4≈0.47)을 그대로 보존.
  function costOf(name,tier){tier=tier||1;const base=UNIT_COST[name],mul=Math.pow(1.55,tier-1),c={};
    for(const r in base)c[r]=Math.round(base[r]*mul);c.철=(c.철||0)+Math.round(2*(tier-1)*mul);return c;}
  // 성 레벨업 비용: 기존엔 레벨 무관 고정값이었음 → 아발론 참조로 레벨당 복리(×1.32) 성장 함수화(레벨1은 구값과 동일, 명시적 상한 없이 자연 소프트캡).
  const CASTLE_UP_BASE={목재:20,석재:15,철:10}, CASTLE_LV_GROWTH=1.32;
  function castleUpCost(level){ level=level||1; const mul=Math.pow(CASTLE_LV_GROWTH, level-1), c={};
    for(const r in CASTLE_UP_BASE) c[r]=Math.round(CASTLE_UP_BASE[r]*mul);
    c.철=(c.철||0)+Math.round(3*(level-1)); return c; }
  ...
  // TIME_SCALE(×30, 위협 시계 동반 스케일 — 생산이 30배 느려진 만큼 웨이브 간격/유예/AI 티어 성장도 같이 늘려야
  // 부대를 미처 못 만든 채 계속 털리는 회귀를 피함, 2026-07-21 확인). budgetPerTurn/graceBudget(턴당 본대 회복 예산)은
  // 반대로 ÷TIME_SCALE — 그대로 두면 공격 간격이 30배 벌어진 사이 본대가 매번 완전 재생해 정복이 원천봉쇄됨(실측으로 확인).
  const AI={budgetPerTurn:5/TIME_SCALE,waveSize:38,homeCap:70,tierEvery:16*TIME_SCALE,waveCap:2,waveFrac:0.85,graceTurns:30*TIME_SCALE,graceBudget:2/TIME_SCALE,waveGap:16*TIME_SCALE}, AI_UNIT_COST={중갑보병:3,창병:3,장궁병:2,석궁병:3,경기병:4,중기병:5};
  // 시즌형 침공(B2): N턴마다 예고→도래하는 대규모 원정대. 도래마다 간격이 줄고(escalating) 규모가 커짐(횟수·국력 둘 다 반영).
  const SEASON_INTERVAL=60*TIME_SCALE, SEASON_MIN_INTERVAL=30*TIME_SCALE, SEASON_WARN_LEAD=12*TIME_SCALE, SEASON_BASE=30, SEASON_GROWTH=0.35;
  ...
  const FACTIONS=[
    {id:"raider",   name:"도적단",   units:["경기병","중기병"],interval:72*TIME_SCALE,base:12,growth:0.22},
    {id:"horde",    name:"오크 군세", units:["중갑보병","창병"],interval:96*TIME_SCALE,base:15,growth:0.25},
    // prey형: 성이 아니라 야외에 나가있는 아군 부대(채집대 등)를 노림 — "그냥 다 성으로 몰려온다"는 반복감을 깨고,
    // 채집·원정 나간 부대를 방치하면 위험하다는 새로운 판단(호위/철수)을 만듦.
    {id:"nightraid",name:"야습대",   units:["장궁병","석궁병"],interval:80*TIME_SCALE,base:10,growth:0.2,prey:true},
  ];
  ...
  const RANK_SEASON_LEN=180*TIME_SCALE, RANK_SOFTRESET=0.35, THREAT_CAP=1.5;
  const RANK_TIERS=[
  ...
  // 연구 트리 보강(2026-07-21): 건설(buildDur)엔 훈련(buildRate)과 달리 가속 수단이 없었음 — 신규 2단으로 메움(buildDur에서 hasR로 직접 소비).
  RESEARCH["건축 공법 I"]={cat:"내정",sub:"경제",req:[],cost:{목재:30,석재:20},turns:3,desc:"건설 소요시간 -10%"};
  RESEARCH["건축 공법 II"]={cat:"내정",sub:"경제",req:["건축 공법 I"],cost:{목재:40,석재:30},turns:4,desc:"건설 소요시간 -10%p (누적)"};
  // 결전/철벽(위 E4)에서 끝나던 트리를 한 단계 더 — 이미 배타적으로 갈렸으므로 excludes 불필요(결전을 골랐으면 결전 II만 해금).
  (function(){ for(const g in GROUPS){
    const kAtk=`${g} 특화: 결전`, kDef=`${g} 특화: 철벽`;
    RESEARCH[`${g} 특화: 결전 II`]={cat:"전투",sub:g,req:[kAtk],cost:{목재:55,철:70},turns:6,desc:`${g} 공격 +15%p (결전 심화, 누적)`,mod:{group:g,stat:"atk",mul:1.15}};
    RESEARCH[`${g} 특화: 철벽 II`]={cat:"전투",sub:g,req:[kDef],cost:{목재:55,철:70},turns:6,desc:`${g} 방어 +15%p (철벽 심화, 누적)`,mod:{group:g,stat:"df",mul:1.15}};
  } })();
  // 아발론 참조 시간배율(TIME_SCALE): 개별 turns 리터럴을 일일이 바꾸지 않고 여기서 일괄 적용(드리프트 방지).
  for(const k in RESEARCH) RESEARCH[k].turns=(RESEARCH[k].turns||1)*TIME_SCALE;
  ...
  const RESPAWN_DELAY=8*TIME_SCALE;
  ...
  let NODES, EDGES, ADJ, MONSTERS, MSPAWN, GRID_BOUNDS;
  function setMap(m){ NODES=m.NODES; EDGES=m.EDGES; MONSTERS=m.MONSTERS||[]; MSPAWN=m.spawn||null;
    ...
    // King of Avalon 참조(맵 확장+중심 편향 티어): 그리드 좌표(gx/gy)가 있는 노드맵(타일맵)만 경계를 계산 —
    // 구버전 노드그래프(DEFAULT_MAP)는 gx/gy가 없어 GRID_BOUNDS=null → 중심편향 완전 비활성(하위호환 보장).
    const gxs=[],gys=[]; for(const id in NODES){const n=NODES[id]; if(n.gx!==undefined){gxs.push(n.gx);gys.push(n.gy);}}
    GRID_BOUNDS = gxs.length ? {cx:(Math.min(...gxs)+Math.max(...gxs))/2, cy:(Math.min(...gys)+Math.max(...gys))/2,
      halfW:(Math.max(...gxs)-Math.min(...gxs))/2||1, halfH:(Math.max(...gys)-Math.min(...gys))/2||1} : null;
    if(typeof API!=="undefined"&&API){ API.NODES=NODES; API.EDGES=EDGES; API.ADJ=ADJ; API.MONSTERS=MONSTERS; } // 노출 참조 갱신
    return API; }
  setMap(DEFAULT_MAP);
  // 랜덤 스폰(타일 맵): 성/인접이 아닌 빈 평지 타일에 몬스터·자원 배치(둘 다 공유하는 후보 필터)
  ...
  // 중심 편향(King of Avalon 참조): 체비셰프 정규화 거리 0(정중앙)~1(가장자리). 그리드 없는 맵은 0.5(중립) 반환.
  function tileDistNorm(id){ if(!GRID_BOUNDS) return 0.5;
    const n=NODES[id]; if(!n||n.gx===undefined) return 0.5;
    const dx=Math.abs(n.gx-GRID_BOUNDS.cx)/GRID_BOUNDS.halfW, dy=Math.abs(n.gy-GRID_BOUNDS.cy)/GRID_BOUNDS.halfH;
    return Math.min(1, Math.max(dx,dy)); }
  const TIER_WEIGHTS_BASE=[40,25,15,10,5];   // T1~T5 기본 희소성(정중앙에서도 T5는 T1보다 8배 희귀)
  const TIER_BIAS_K=1.5;                     // 거리에 따른 감쇠 강도 — 가장자리로 갈수록 고티어 급격히 희귀해짐
  function pickTierByDistance(distNorm){
    const w=TIER_WEIGHTS_BASE.map((base,i)=>base*Math.pow(1-distNorm, TIER_BIAS_K*i));
    const sum=w.reduce((a,b)=>a+b,0), r=Math.random()*sum; let acc=0;
    for(let i=0;i<w.length;i++){ acc+=w[i]; if(r<=acc) return i+1; } return 1; }
  ...
  // ---- King of Avalon 참조: 몬스터 티어(1~5) — 자원 타일과 동일한 5단계·중심편향 축 공유 ----
  // 기존 5종(늑대/고블린/오크/하피/오우거, engine.js 변경 없음)을 재사용, uk()로 comp 키에 티어를 굽는다(예:"오크@T3").
  // mtier(사냥/토벌/레이드)는 티어에서 파생 — 기존 보상 테이블(XP_REWARD 등, 전부 이 3라벨 키)은 변경 없이 그대로 재사용.
  const MONSTER_TIERS=[
    {tier:1, mtier:"사냥", variants:[
      {name:"늑대 무리",     comp:{늑대:10},              reward:{식량:20,목재:15}},
      {name:"들개 무리",     comp:{늑대:6,고블린:4},       reward:{식량:18,철:10}} ]},
    {tier:2, mtier:"사냥", variants:[
      {name:"고블린 약탈대", comp:{고블린:14},             reward:{식량:24,목재:18}},
      {name:"도적 무리",     comp:{고블린:8,늑대:6},       reward:{식량:22,철:14}} ]},
    {tier:3, mtier:"토벌", variants:[
      {name:"오크 전투대",   comp:{오크:8,하피:4},         reward:{철:38,석재:26,식량:26}},
      {name:"고블린 소굴",   comp:{오크:6,하피:6,고블린:6}, reward:{철:34,석재:24,식량:24}} ]},
    {tier:4, mtier:"토벌", variants:[
      {name:"오크 정예대",   comp:{오크:10,오우거:3,하피:5}, reward:{철:46,목재:32,석재:28}} ]},
    {tier:5, mtier:"레이드", variants:[
      {name:"고대 파수병",   comp:{오우거:6,오크:10,하피:6}, reward:{철:60,석재:44,식량:44}} ]},
  ];
  const tieredComp=(comp,tier)=>tier<=1?{...comp}:Object.fromEntries(Object.entries(comp).map(([k,v])=>[uk(k,tier),v]));
  function monsterTemplateFor(tier){ const t=MONSTER_TIERS[Math.max(0,Math.min(4,tier-1))]; return t.variants[Math.floor(Math.random()*t.variants.length)]; }
  // 랜덤 스폰(타일맵 전용): 뽑힌 타일의 중심거리로 먼저 티어를 정하고, 그 티어의 변형 템플릿을 고름 — pool 인자 제거(내부 자체 결정).
  function spawnRoamer(g){ const node=randomTile(g); if(!node) return null;
    const tier=pickTierByDistance(tileDistNorm(node)), tmpl=monsterTemplateFor(tier);
    const a={id:"MON"+(g._nid++),side:"M",node,mp:0,maxMp:0,name:tmpl.name,mtier:tmpl.mtier,tier,
      comp:scaleComp(tieredComp(tmpl.comp,tier), monsterScale(g)), hero:null, reward:{...tmpl.reward}, roamer:true};
    g.armies.push(a); return a; }

  // ---- King of Avalon 참조: 자원 타일 티어(1~5) + 고갈 + 재등장 — "야생" 타일 전용(집 근처 고정 8타일은 무한 산출 그대로) ----
  const RES_TIER_YIELD=[6,10,16,26,42];             // T1=기존 GATHER_BASE와 연속성. 이후 ~1.6배/단계(아발론 Gather 표 §8-1 참조)
  const RES_TIER_CAP=RES_TIER_YIELD.map(y=>y*12);   // 완전 소진까지 상주 채집 기준 약 12턴
  const RESPAWN_DELAY_RES=10*TIME_SCALE;             // 몬스터 둥지(8턴)보다 살짝 느긋한 경제형 재생성
  function plantResourceTile(id){ const tier=pickTierByDistance(tileDistNorm(id));
    const res=RES[Math.floor(Math.random()*RES.length)], cap=RES_TIER_CAP[tier-1], g0=NODES[id];
    NODES[id]={name:{식량:"야생 곡창",목재:"야생 삼림",석재:"야생 채석장",철:"야생 철광"}[res],
      type:"resource", res, tier, cap, remaining:cap, gx:g0.gx, gy:g0.gy}; }
  function seedWildResources(g,count){ for(let i=0;i<count;i++){ const id=randomTile(g); if(id) plantResourceTile(id); } }
  function respawnResourceTile(g){ const id=randomTile(g);
    if(!id){ (g.respawns=g.respawns||[]).push({resTile:true,at:g.turn+2}); return; }   // 빈 평지 없음 → 재시도 지연
    plantResourceTile(id); }
  function depleteTile(g,nodeId,amt){ const n=NODES[nodeId]; if(!n||n.remaining===undefined) return;   // 고정 타일=no-op(하위호환)
    n.remaining=Math.max(0,n.remaining-amt);
    if(n.remaining<=0){ NODES[nodeId]={name:"",type:"plain",gx:n.gx,gy:n.gy};
      (g.respawns=g.respawns||[]).push({resTile:true, at:g.turn+RESPAWN_DELAY_RES}); } }
  ...
  function newGame(){ const g=_baseGame();
    if(MSPAWN){ for(let i=0;i<(MSPAWN.count||4);i++) spawnRoamer(g);
      if(MSPAWN.raid && NODES.ANCIENT) g.armies.push(mkMonster({node:"ANCIENT",...MSPAWN.raid},g));
      if(MSPAWN.wildResCount) seedWildResources(g, MSPAWN.wildResCount); }
  ...
  function gatherOf(g,a){const n=NODES[a.node];if(a.side!=="P"||n.type!=="resource")return null;
    ...
    // King of Avalon 참조: 야생 타일(n.tier 존재)은 티어별 산출량, 집 근처 고정 타일은 기존 GATHER_BASE 그대로(하위호환).
    const tierBase=n.tier?RES_TIER_YIELD[n.tier-1]:GATHER_BASE;
    const base=tierBase+(hasR(g,"채굴법")?2:0)+(hasR(g,"정밀 채굴")?2:0);
    const mult=isCity?GRADE_GATHER[hero.grade]+traitSum(hero,"gatherBonus"):1;
    let amt=Math.round(base*mult);
    if(n.remaining!==undefined) amt=Math.min(amt,n.remaining);   // 고갈 직전엔 남은 만큼만
    return {res:n.res,amt};}
  ...
  function income(g){const inc={식량:0,목재:0,석재:0,철:0},base=castleBaseIncome(g),e=econIncome(g);
    ...
    return inc;}
  // income()은 UI 자원바 미리보기(렌더마다 호출)에도 쓰이는 순수 계산 함수라 여기서 고갈시키면 안 됨 —
  // 실제 턴 처리(endTurn/offlineStep)에서만 별도로 호출해 채집한 만큼 정확히 1턴에 1번 차감.
  function applyGatherDepletion(g){ for(const a of g.armies){ const gt=gatherOf(g,a); if(gt) depleteTile(g,a.node,gt.amt); } }
  ...
  function processRespawns(g){ if(!g.respawns||!g.respawns.length)return; const keep=[];
    for(const r of g.respawns){ if(r.at>g.turn){keep.push(r);continue;}
      ...
      if(r.resTile){ respawnResourceTile(g); continue; }                             // 야생 자원 타일 재등장(King of Avalon 참조)
      if(r.random){ if(MSPAWN)spawnRoamer(g); continue; }  // 랜덤 타일 재등장(시그니처 변경 — pool 인자 제거, 내부에서 중심편향 티어 자체 결정)
  ...
  // 한 칸 진입 + 교전 해결. {battle} 반환. 유저 요청: 몬스터/적 타일을 그냥 "지나가기만" 해도 전투가 발생하던 문제 —
  // 경유지(중간 통과 타일)는 접촉해도 싸우지 않고, 목적지(a.dest)에 실제로 도착·정지할 때만 교전 판정.
  function enterTile(g,a,next,isDest){ a.node=next; let battle=null;
    if(isDest){
  ...
  function moveTick(g){ let battle=null, event=null;
    for(const a of [...g.armies]){ if(g.over) break;
      ...
      while(a.node!==a.dest && guard++<50){
        ...
        a.moveProg-=cost;
        const r=enterTile(g,a,next,next===a.dest);
  ...
  // 아발론 참조: 기존엔 castle/bld/wall도 레벨 무관 고정 소요턴이었음 → 레벨이 오를수록 건설도 오래 걸리게(레벨1은 구값과 동일).
  // TIME_SCALE(×30) 적용 + "건축 공법 I/II" 연구로 -10%p씩 단축(훈련엔 buildRate 가속이 있었지만 건설엔 없던 공백을 메움).
  const buildDur=(g,kind,lv)=>{ lv=lv||0; let t;
    if(kind==="castle")t=4+Math.floor(lv/3);
    else if(kind==="bld")t=3+Math.floor(lv/2);
    else if(kind==="wall")t=3+Math.floor(lv/3);
    else if(kind==="construct")t=3; else if(kind==="univ")t=3; else if(kind==="tavern")t=2;
    else t=2+Math.floor(lv/2);
    const cut=(hasR(g,"건축 공법 I")?0.1:0)+(hasR(g,"건축 공법 II")?0.1:0);
    return Math.max(1, Math.round(t*TIME_SCALE*(1-cut))); };
  ...
  function construct(g,key){if(g.castle.buildings.includes(key))return null; return startBuild(g,"construct",key,BUILDINGS[key].cost,buildDur(g,"construct"),`${key} 건설`);}
  function upgradeBuilding(g,key){if(!g.castle.buildings.includes(key))return"미건설";
    const lv=g.castle.blevel[key]||1; if(lv>=tierCap(g))return"최대 레벨(마일스톤 해금 필요)"; return startBuild(g,"bld",key,bUpCost(key,lv),buildDur(g,"bld",lv),`${key} → T${lv+1}`);}
  function fortifyWall(g){return startBuild(g,"wall",null,wallCost(g.castle.wall||0),buildDur(g,"wall",g.castle.wall||0),"성벽 보강");}
  ...
  function levelUp(g){return startBuild(g,"castle",null,castleUpCost(g.castle.level),buildDur(g,"castle",g.castle.level),"성 레벨업");}
  function buildEcon(g,k){const lv=g.castle.econ[k]||0; if(lv>=ECON_MAX)return"최대 레벨"; return startBuild(g,"econ",k,econCost(k,lv),buildDur(g,"econ",lv),`${k} Lv${lv+1}`);}
  function buildUniversity(g){if(g.castle.buildings.includes("대학"))return null; return startBuild(g,"univ",null,UNIV_COST,buildDur(g,"univ"),"대학 건설");}
  function startResearch(g,k){if(g.research.active)return"이미 연구 중"; const r=RESEARCH[k]; if(g.research.done[k])return null;
    ...
    pay(g,r.cost);
    // TIME_SCALE로 turns가 30배 커진 만큼, 턴 감소 보너스(영웅특성/재능/장비의 flat -1~-6)도 같이 스케일해야 비율이 보존됨.
    const rBonus=(councilSum(g,"researchBonus")+lordTalentSum(g,"researchBonus")+lordEquipSum(g,"researchBonus"))*TIME_SCALE;
    const turns=Math.max(1,r.turns-rBonus);
  ...
  function buildTavern(g){if(g.tavern.built)return null; return startBuild(g,"tavern",null,TAVERN_COST,buildDur(g,"tavern"),"선술집 건설");}
  ...
  function aiTurn(g){
    ...
    g.ai.budget+=(grace?AI.graceBudget:AI.budgetPerTurn)+Math.min(3,Math.floor(g.turn/(14*TIME_SCALE)))/TIME_SCALE;   // 후반 눈덩이 완화(긴 판 대응) — 이것도 턴당 flat rate라 ÷TIME_SCALE
  ...
  function seasonTick(g){
    ...
    if(g.turn>=s.next){
      ...
      const interval=Math.max(SEASON_MIN_INTERVAL, SEASON_INTERVAL-3*TIME_SCALE*(s.count-1));
  ...
  function endTurn(g){
    ...
    const inc=income(g); for(const r of RES)g.res[r]+=inc[r]; applyGatherDepletion(g);
  ...
  API={ RES,GATHER_BASE,GATHER_HERO,ARMY_CAP,ECON_CAP,WOUND_RATE,HP_SCALE,UNIT_COST,castleUpCost,BUILDINGS,ECON_BUILDINGS,UNIV_COST,GROUPS,STATNAME,AI,AI_UNIT_COST,RESEARCH,NODES,EDGES,ADJ,
